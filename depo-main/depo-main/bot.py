...
async def new_project(ctx):
    ...
    link = await bot.wait_for('message', check=check)                               #!new_project ile komut çalışır
    data.append(link.content)                                                       # id ve isim alınır ve link eklenir
                                                                                    # get_status ile veriler kullanıcıya aktarılır
                                                                                    #kullanıcının seçimine göre durum data isimli listeye eklenir
    statuses = [x[0] for x in manager.get_statuses()]                               #insert_project(tuple(data)) diyerek de ekleme işlemi gerçekleştirilir.
...
async def get_projects(ctx):
    user_id = ctx.author.id                                             # projelerin hepsini döndürür
    projects = manager.get_projects(user_id)                            #proje ismi ve linkini indeksler aracılığıyla getirir.
    if projects:
...
async def skills(ctx):
    ...
    if projects:
        projects = [x[2] for x in projects]                             # 2. indeksten isimlere erişerek kullanıcıya hangi proje üzerinde çalışmak istediğini soruyor
        await ctx.send('Bir beceri eklemek istediğiniz projeyi seçin')  #1. indeksten hangi yetenekleri verebileceğime eriştim.
        await ctx.send("\n".join(projects))                             #insert_skill methodu ile ekleme işlemi gerçekleşti.
...
async def delete_project(ctx):
    ...
    if projects:
        ...
        await ctx.send("\n".join(projects))
                                                                            # Proje ismine göre hangisinin silineceği belirlenir
        def check(msg):                                                     # kullancı id ve proje id sine göre delete_project üzerinden silme gerçekleştirilir.
            return msg.author == ctx.author and msg.channel == ctx.channel
...
async def update_projects(ctx):
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:                                                        # keyler üzerinden projede neyi değiştirmek istediğimizi seçiyoruz
        projects = [x[2] for x in projects]                             # update projects methodu üzerinden güncelleme işlemi gerçekleştirilir.
